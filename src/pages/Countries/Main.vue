      ...
      <q-card bordered>
        <q-card-section>
          <q-breadcrumbs>
            ...
            <q-breadcrumbs-el label="Countries" icon="fas fa-globe-asia" />
          </q-breadcrumbs>
        </q-card-section>
        <q-separator inset />
...
export default {
  ...
  methods: {
    ...
    ...mapActions('showcase', ['fetchDataCountry', 'deleteDataCountry']),
    loadingShow() {
    ...
    onDelete() {
      this.$q.loading.show()
      this.loading = true
      this.deleteDataCountry(this.selected[0].id_country)
      setTimeout(() => {
        this.$q.loading.hide()
        this.loading = false
        this.$q.notify({
    ...
    onUpdate() {
      ...
      this.$router.push({ path: '/countries/edit' })
    },
    onView() {
      localStorage.setItem('selectedData', JSON.stringify(this.selected[0]))
      this.$router.push({ path: '/countries/view' })
    },
